...
#-----------------------------------------------------------------------
def main():
    diri = '/global/cscratch1/sd/zhan391/DARPA_project/post_process/trainning_data/SE_PG2' #-- data directory
    varName = 'U'
    varDesc = 'Zonal wind'
    varUnit = 'm/s'
    itime   = 0
    ...
    expname.append('Before Nudging')
    filname.append(diri+'/before_nudging/ML_IN_E3SMv2_NDGUVTQ_tau6_3hourly_200911302100-201011301800.nc')
    expname.append('After Nudging')
    filname.append(diri+'/after_nudging/ML_IN_E3SMv2_NDGUVTQ_tau6_3hourly_200911302100-201011301800.nc')
    expname.append('Reference solution')
    filname.append(diri+'/reference_data/ML_REF_ERA5_3hourly_200911302100-201011301800.nc')
    expname.append('Nudging tendency')
    ...
    for i in range(len(expname)):
       ...
       #res.lbTitleFontHeightF        = fontsize
        ...
        res.sfXArray              =  lon.values    #-- transform x to mesh scalar field
        res.sfYArray              =  lat.values    #-- transform y to mesh scalar field
        #res.sfXCellBounds         =  lon.values   #-- needed if set cnFillMode = "CellFill"
        #res.sfYCellBounds         =  lat.values   #-- needed if set cnFillMode = "CellFill"
